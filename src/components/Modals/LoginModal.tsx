...
const LoginModal = () => {
    ...
    const onToggleStep2 = useCallback(() => { setStep(3); }, [setStep]);
    const onToggleStep3 = useCallback(() => { setStep(2); }, [setStep]);
    ...
    if (step === 1) {
                ...
                <div className="w-full">
                    <button className="gsi-material-button w-full" onClick={() => redirectAuthGoogle()}>
                        ...
                        <div className="gsi-material-button-content-wrapper">
                            ...
                            <span className="gsi-material-button-contents">{t('sign_in_google')}</span>
                            <span style={{ display: 'none' }}>{t('sign_in_google')}</span>
                        </div>
                    </button>
                </div>
                <div className="flex flex-row justify-center w-full pt-2 text-sm">
                    <p className="pr-2">{t('no_account')}</p>
                    <span className="cursor-pointer hover:opacity-50 underline" onClick={onToggleStep1}>
                        {t('sign_up')}
                    </span>
                </div>
            </div>
        ...
        footerContent = (
            <div className="flex flex-row py-2">
                <p className="pr-2">{t('problem_2fa')}</p>
                <span className="cursor-pointer hover:opacity-50 underline" onClick={onToggleStep2}>
                    {t('recovery_codes')}
                </span>
            </div>
        );
        ...
        footerContent = (
            <div className="flex flex-row py-2">
                <p className="pr-2">{t('problem_recovery')}</p>
                <span className="cursor-pointer hover:opacity-50 underline" onClick={onToggleStep3}>
                    {t('auth_code')}
                </span>
            </div>
        );
